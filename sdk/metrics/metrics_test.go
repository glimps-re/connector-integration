...
func Test_MetricsCollector_GetAndReset(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if m.itemsMitigated.Load() != 0 {
				t.Errorf("mitigatedItems not reset, got %v", m.itemsMitigated.Load())
			}
			if m.itemsError.Load() != 0 {
				t.Errorf("errorItems not reset, got %v", m.itemsError.Load())
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := ConnectorMetrics{
				...
				ItemsMitigated: m.itemsMitigated.Load(),
				ItemsError:     m.itemsError.Load(),
			}
			if diff := cmp.Diff(tt.want, got); diff != "" {
...
func Test_MetricsCollector_AddErrorItem(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := m.itemsError.Load(); got != tt.wantItemsError {
				t.Errorf("errorItems = %v, want %v", got, tt.wantItemsError)
			}
		})
	}
}
...
func Test_MetricsCollector_AddMitigatedItem(t *testing.T) {
	tests := []struct {
		name               string
		wantItemsMitigated int64
	}{
		{
			name:               "ok",
			wantItemsMitigated: 1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := m.itemsMitigated.Load(); got != tt.wantItemsMitigated {
				t.Errorf("mitigatedItems = %v, want %v", got, tt.wantItemsMitigated)
			}
		})
	}
}
