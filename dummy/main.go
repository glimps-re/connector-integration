...
// getSpeedRate returns the sleep multiplier. It is a float so a rate below 1
// can speed the dummy up, not only slow it down.
...
func getIterationAmount() (iterationAmount int) {
	iterationAmount = -1
	value := os.Getenv("ITERATION_AMOUNT")
	if value == "" {
		return
	}
	iterationAmount, err := strconv.Atoi(value)
	if err != nil {
		logger.Warn("invalid ITERATION_AMOUNT, running without iteration limit", slog.String("value", value))
		iterationAmount = -1
	}
	return
}
...
func getEmitAllMitigationReasons() (emitAll bool) {
	value := os.Getenv("EMIT_ALL_MITIGATION_REASONS")
	if value == "" {
		return
	}
	emitAll, err := strconv.ParseBool(value)
	if err != nil {
		logger.Warn("invalid EMIT_ALL_MITIGATION_REASONS, not emitting all reasons", slog.String("value", value))
		emitAll = false
	}
	return
}
...
//
// Each reason gets a distinct count so a test can check the reason.
// 7 is skipped because the console merges invalid into error and
// displays 3 + 4 = 7.
		...
		for {
			...
			default:
				...
				if iterationAmount != -1 && count >= iterationAmount {
					...
					<-ctx.Done() // idle until cancellation
					continue
				}
				count++
